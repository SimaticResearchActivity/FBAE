...
namespace fbae_test_BBOBB {
    ...
    TEST_F(BBOBBTest, ExecuteWith2SitesAndRank1ReceiveStepInNextWaveThenStepInWave) {
        nbSites = 2;
        sites = vector<HostTuple>(nbSites, HostTuple{"", 0});
        Arguments arguments{sites};
        myRank = 1;
    ...
    TEST_F(BBOBBTest, ExecuteWith8SitesAndRank1ReceiveStepsWhichDoNotTriggerImmediatlySendingOfmessages) {
        /*
        ...
        *      ==> No reaction of site with rank 1
        ...
        ASSERT_EQ(2, stepMsg0.batchesBroadcast.size()); // In current BBOBB implementation, we store batches of messages even though batches of messages is empty
