...
void AlgoStub::callbackInitDone() {
    initDoneSupervisor.callbackInitDone();
}
...
std::vector<std::string> &AlgoStub::getReceived() {
    return received;
}

void AlgoStub::totalOrderBroadcast(const fbae_SessionLayer::SessionMsg &sessionMsg) {
    // No sense to call this method in the context of tests.
    assert(false);
}
