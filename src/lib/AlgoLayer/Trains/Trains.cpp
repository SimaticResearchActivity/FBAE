...
void Trains::processTrain(string&& serializedMessagePacket) {
    ...
    vector<fbae_AlgoLayer::BatchSessionMsg> batchesToDeliver = previousTrainsBatches[train.id];
...
void Trains::execute() {
    ...
    if (const short N = 128; nbTrains > N) {
        LOG4CXX_FATAL_FMT(getAlgoLogger(), "Number of train ({}) should be inferior to {}", nbTrains, N);
        exit(EXIT_FAILURE);
    }
...
bool Trains::isTrainRecent(uint8_t trainId, uint8_t trainClock) {
    ...
    if (trainId == idExpected) {
        ...
        LOG4CXX_FATAL_FMT(getAlgoLogger(), "Rank #{:d}: Train expected: {:d}, Train got: {:d}", rank, idExpected, trainId);
        return false; }
}
