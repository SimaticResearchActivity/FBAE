...
Arguments::Arguments(fbae::core::OptParserExtended const& parser)
  ...
  if (parser.hasopt('f')) {
    ...
    if (frequency == 0) {
      LOG4CXX_FATAL_FMT(
          logger, "Argument for frequency must be greater than 0 (zero) \n {}",
          parser.synopsis());
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (parser.hasopt('m')) {
    ...
    if (maxBatchSize < sizeMsg) {
      LOG4CXX_FATAL_FMT(logger,
                        "Argument for maxBatchSize must be greater than "
                        "argument for sizeMsg \n {}",
                        parser.synopsis());
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (parser.hasopt('P')) {
    networkLevelMulticastPort =
        static_cast<uint16_t>(parser.getoptIntRequired('P', logger));
  }
  ...
  if (parser.hasopt('w')) {
    ...
    if (warmupCooldown > 99) {
      LOG4CXX_FATAL_FMT(logger,
                        "Argument for warmupCooldown must be in [0,99] \n {}",
                        parser.synopsis());
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (sizeMsg < minSizeClientMessageToBroadcast || sizeMsg > maxLength) {
    LOG4CXX_FATAL_FMT(logger,
                      ...
                      "interval [ {} , {} ] \n {}",
                      sizeMsg, minSizeClientMessageToBroadcast, maxLength,
                      parser.synopsis());
    exit(EXIT_FAILURE);
  }
  ...
  if ((rank != specialRankToRequestExecutionInTasks) &&
      (rank > sites.size() - 1)) {
    LOG4CXX_FATAL_FMT(logger,
                      "You specified a rank of {}, but there are only {} sites "
...
[[nodiscard]] std::string Arguments::asCsv(std::string const& algoStr,
  ...
  return std::format("{},{},{},{},{},{},{},{:d},{}%,{},{},{}", algoStr,
                     algoArgument, commLayerStr, commArgument,
                     (usingNetworkLevelMulticast ? "true" : "false"), frequency,
                     maxBatchSize, nbMsg, warmupCooldown, rankStr, sizeMsg,
                     siteFile);
}

std::string Arguments::csvHeadline() {
  return std::string{
      "algoLayer,algoArgument,commLayer,commArgument,"
      "isUsingNetworkLevelMulticast,frequency,maxBatchSize,nbMsg,"
      "warmupCooldown,rank,sizeMsg,siteFile"};
}

int Arguments::genericGetIntInArgument(std::string_view searchedArgument,
                                       int defaultIntValue,
                                       std::string_view arg,
  ...
  if (posSearchedArgument == std::string::npos) {
    LOG4CXX_INFO_FMT(
        logger,
        "Option --{} does not contain {} argument ==> Using default value {}",
        argName, searchedArgument, defaultIntValue);
    return defaultIntValue;
  }
  auto posVal = posSearchedArgument + searchedArgument.size() +
                1;  // + 1 to take into account size of '='
  auto valAsString{
      argWithoutSpace.substr(posVal, argWithoutSpace.size() - posVal)};
  std::istringstream iss(valAsString);
  int val;
  iss >> val;
  if (!iss) {
    LOG4CXX_WARN_FMT(logger,
                     "Option --{} contains {} argument which is not followed "
                     "by numeric value ==> Using default value {}",
                     argName, searchedArgument, defaultIntValue);
    return defaultIntValue;
  }
  LOG4CXX_INFO_FMT(
      logger,
      "Option --{} contains {} argument with value {} which will be used",
      argName, searchedArgument, val);
  return val;
}

int Arguments::getIntInAlgoArgument(std::string_view searchedArgument,
                                    int defaultIntValue) const {
  return genericGetIntInArgument(searchedArgument, defaultIntValue,
                                 algoArgument, "AlgoArgument");
}

int Arguments::getIntInCommArgument(std::string_view searchedArgument,
                                    int defaultIntValue) const {
  return genericGetIntInArgument(searchedArgument, defaultIntValue,
                                 commArgument, "CommArgument");
}

int Arguments::getFrequency() const { return frequency; }

int Arguments::getMaxBatchSize() const { return maxBatchSize; }

int64_t Arguments::getNbMsg() const { return nbMsg; }
...
rank_t Arguments::getRank() const { return rank; }

std::vector<std::tuple<std::string, int>> Arguments::getSites() const {
  return sites;
}

int Arguments::getSizeMsg() const { return sizeMsg; }

int Arguments::getWarmupCooldown() const { return warmupCooldown; }
