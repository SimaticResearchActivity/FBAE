/*!
  ...
  Project found at
https://www.codeproject.com/Articles/5351070/Cplusplus-Replacement-for-getopt
  License: MIT
...
```C
    const char *optlist[] {
      ...
      "c+ one_or_more_args \t -c can be followed by one or more arguments
example: -c 12 ab cd", "d* 0_or_more_args \t -d can have zero or more
arguments", "e| \t -e doesn't have any arguments", "f?longorshort optional \t -f
can be also written as --longorshort",
...
class OptParser {
 public:
  ...
  int getopt(const std::string& option, std::string& optarg,
             char sep = '|') const;
...
*/
inline const std::string& OptParser::appname() const { return app; }
...
inline int OptParser::getopt(const std::string& option,
                             std::vector<std::string>& optarg) const {
  optarg.clear();
  auto p = find_option(option);
  if (p == cmd.end()) return 0;

  optarg = p->arg;
  return p->count;
}

inline int OptParser::getopt(char option,
                             std::vector<std::string>& optarg) const {
  optarg.clear();
  auto p = find_option(option);
  if (p == cmd.end()) return 0;
