...
class AlgoLayer {
 public:
  ...
  /**
   * @brief Callback to be called by @AlgoLayer when @AlgoLayer is using
   ...
   * value of @batchCtrlShortcut (it will always wait for
   * @batchWaitingSessionMsg to be small enough; See Issue #37).
   */
  ...
  /**
   * @brief Callback called by @CommLayer when @CommLayer is initialized
   * locally.
   */
  virtual void callbackInitDone();

  /**
   * @brief Executes concrete Total Order Broadcast algorithm. Returns when
   * algorithm is done.
   */
   ...
   * vector.
   * @return Position in @broadcasters vector if participant is indeed a
   * broadcasting participant and nullopt if participant is not a broadcasting
   * participant.
   */
   ...
   * @return true if the execution of @ALgoLayer lead to the production of
   * statistics (e.g. case of a broadcaster in Sequencer algorithm) and false
   * otherwise (e.g. which can be the case of the sequencer in Sequencer
   * algorithm of the sequencer is not a broadcaster)
   */
   ...
   * not override, the message is stored in
   * @batchWaitingSessionMsg
   ...
   * Eventually this call will lead to the return of @execute method.
   */
 ...
 private:
  ...
  /**
   * @brief Condition variable coupled with @batchCtrlMtx to control that batch
   * of messages in
  ...
  /**
   * @brief Mutex coupled with @batchCtrlCondVar to control that batch of
   * messages in
   * @batchWaitingSessionMsg is not too big
   */
  ...
  /**
   * @brief Variable used to shortcut BatchCtrl mechanism (with @batchCtrlMtx
   * and @batchCtrlCondVar), so that, in order to avoid deadlocks, we accept
   * that the number of bytes stored in @batchWaitingSessionMsg is greater than
   * @maxBatchSize of @Arguments instance.
   */
  bool batchCtrlShortcut{false};

  /**
   * @brief Vector of threads which registered to accept that they are not
   * concerned by value of @batchCtrlShortcut (they always wait for
   * @batchWaitingSessionMsg to be small enough; See Issue #37).
   */
