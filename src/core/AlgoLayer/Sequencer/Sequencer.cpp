...
void Sequencer::callbackReceive(std::string &&algoMsgAsString) {
  ...
  switch (msgId) {
    ...
    case BroadcastRequest: {
      auto msgToBroadcast{deserializeStruct<StructBroadcastMessage>(
          std::move(algoMsgAsString))};
      auto s{serializeStruct<StructBroadcastMessage>(
          StructBroadcastMessage{MsgId::Broadcast, msgToBroadcast.senderPos,
    ...
    //
    case Broadcast: {
      auto sbm{deserializeStruct<StructBroadcastMessage>(
          std::move(algoMsgAsString))};
      getSessionLayer()->callbackDeliver(sbm.senderPos,
                                         std::move(sbm.sessionMsg));
      break;
    }
    default: {
...
void Sequencer::execute() {
  ...
  if (getSessionLayer()->getRank() == sequencerRank) {
    ...
    LOG4CXX_INFO(getAlgoLogger(),
                 "Finished waiting for messages ==> Giving back control to "
                 "SessionLayer");
  } else {
    ...
    LOG4CXX_INFO_FMT(getAlgoLogger(),
                     ...
                     "messages ==> Giving back control to SessionLayer",
                     getSessionLayer()->getRank());
  }
}

void Sequencer::terminate() { getCommLayer()->terminate(); }
