...
enum class SessionMsgId : MsgId_t {
  ...
  FinishedPerfMeasures,  /// Broadcast message signifying its sender has
                         /// finished to send all of its @PerfMeasure messages.
  FirstBroadcast,  /// Broadcast message used by a sender when it broadcasts a
                   /// message for the first time.
  PerfMeasure,     /// Broadcast message used by a sender as Ping message
  PerfResponse,    /// Broadcast message used as a Pong message in response to a
                   /// @PerfMeasure message
...
struct SessionBaseClass {
  ...
  /**
   * @brief Method to facilitate unit tests where it is required to get payload
   * field of @Sessiontest messages. It should never be calles for other types
   * od Session messages.
...
struct SessionPerf : SessionBaseClass {
  SessionPerf(SessionMsgId msgId, rank_t senderPos, int32_t msgNum,
 ...
 private:
  ...
  template <class Archive>
  void serialize(Archive& archive) {
    archive(cereal::base_class<SessionBaseClass>(this), senderPos, msgNum,
            sendTime, filler);
  }
};
...
struct SessionTest : SessionBaseClass {
  SessionTest(SessionMsgId msgId, std::string payload)
      : SessionBaseClass{msgId}, payload{std::move(payload)} {}
...
// the full names (including the namespace) is not used when the name is stored
