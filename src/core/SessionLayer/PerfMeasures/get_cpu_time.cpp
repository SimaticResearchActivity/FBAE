...
//  Windows
#ifdef _WIN32  // A 32-bit platform. This value is also defined by the 64-bit
               // compiler for backward compatibility. (see
...
unsigned long long fbae::core::SessionLayer::PerfMeasures::get_cpu_time() {
  ...
  if (GetProcessTimes(GetCurrentProcess(), &start, &exit, &kernel, &user) !=
    ...
    return ((kernel.dwLowDateTime | (unsigned long long)kernel.dwHighDateTime
                                        << 32) +
            (user.dwLowDateTime | (unsigned long long)user.dwHighDateTime
                                      << 32)) /
           10;
  } else {
    //  Handle error
