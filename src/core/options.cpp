/*!
  ...
  or long options composed of '--' followed by the option name. An option can
have arguments that are either required or optional. If an option can have
multiple arguments, all program arguments following the option are considered
option arguments until the next option argument (argument preceded by '-' or
'--') or until the last argument. The first '--' argument stops options'
processing. All remaining arguments are considered non-option arguments
(operands).
...
```
  ...
  3. Call OptParser::getopt() function to retrieve the status of the various
options
...
```CPP
  const char *optlist[] {
    ...
    "b: required_arg",        // option -b must be followed by an argument
                              // example: -b mmm
    "c+ one_or_more_args",    // option -c can be followed by one or more
arguments
                              // example: -c 12 ab cd.
...
*/
OptParser::OptParser(std::vector<const char*>& list) : nextop(cmd.begin()) {
  set_options(list);
}
...
```
  [<short_form>] <flag> [<long_form>]
[<spaces><arg_description>][\t<opt_description>]
```
  ...
  The `<arg_description>` is the argument description and is used to generate
the synopsis string. The `<opt_description>` is the option description and is
used to generate the description string. The `<opt_description>` part is
...
/*!
  ...
  \param stop   pointer to an integer that receives the index of the first
  non-option argument if function is successful, or the index of the invalid
...
int OptParser::parse(int argc, const char* const* argv, int* stop) {
  ...
  while (i < argc) {
    ...
    switch (op->flag) {
      case '?':  // optional arg
        if (i < argc && *argv[i] != '-') option.arg.push_back(argv[i++]);
        break;
      ...
      case '+':  // one or more
        if (i < argc && *argv[i] != '-') {
          option.arg.push_back(argv[i++]);
          while (i < argc && *argv[i] != '-') option.arg.push_back(argv[i++]);
        } else {
          ret = 2;  // required arg missing
          --i;
          goto done;
...
/*!
  ...
  \param  sep     separator character to insert between arguments if option has
  multiple arguments
  ...
  If the option has multiple arguments, `optarg` contains the arguments
  separated by separator character.
 */
bool OptParser::next(std::string& opt, std::string& optarg, char sep) {
  if (nextop == cmd.end()) return false;
...
/*!
  ...
  The internal position counter is initialized to the first option when command
  line is parsed and is incremented after each call to next() function. Note
  that the internal counter is not thread-safe. Calls to `next()` function from
  any thread increment the same counter.
...
bool OptParser::next(std::string& opt, std::vector<std::string>& optarg) {
  optarg.clear();
  if (cmd.empty() || nextop == cmd.end()) return false;
  if (!nextop->olong.empty())
    opt = nextop->olong;
  else
    opt = nextop->oshort;
...
/*!
  ...
  separated by separator character.
*/
int OptParser::getopt(const std::string& option, std::string& optarg,
                      char sep) const {
  optarg.clear();

  auto op = find_option(option);

  if (op == cmd.end()) return 0;
...
const string OptParser::synopsis() const {
  ...
  for (auto& op : optlist) {
    ...
    if (tabpos != string::npos) {
      param.erase(tabpos);
      param.erase(find_if(param.rbegin(), param.rend(),
...
std::vector<OptParser::opt>::const_iterator OptParser::find_option(
...
void OptParser::format_arg(std::string& str, const opt& option,
                           char sep) const {
  str.clear();
  auto p = option.arg.begin();
  while (p != option.arg.end()) {
    str += *p++;
    if (p != option.arg.end()) str.push_back(sep);
  }
}
