...
TEST_F(TrainsTest, ExecuteWith4SitesAndRank0And1Train) {
  ...
  auto train{
      deserializeStruct<Train>(std::move(commLayerRaw->getSent()[0].second))};
...
TEST_F(TrainsTest, ExecuteWith4SitesAndRank0And4Train) {
  ...
  for (int i = 1; i < nbTrains; i++) {
    auto train{
        deserializeStruct<Train>(std::move(commLayerRaw->getSent()[i].second))};
  ...
  // Check clocks
  ASSERT_EQ(2, algoLayerRaw->getClock(0));
  ASSERT_EQ(2, algoLayerRaw->getClock(1));
  ASSERT_EQ(2, algoLayerRaw->getClock(2));
  ASSERT_EQ(2, algoLayerRaw->getClock(3));
}
...
static string buildTrain(uint8_t trainId) {
  return serializeStruct<Train>(Train{trainId, 3, {}});
}
...
TEST_F(TrainsTest, DeliverMessages) {
  ...
  ASSERT_EQ(1,
            algoLayerRaw->getWaitingBatchesNb());  // Only the first broadcast
                                                   // is waiting for deliver

  // Waiting batches
  auto batch1 = buildBatch(myRank, payloadA);
  auto batch2 =
      buildBatch(static_cast<rank_t>((myRank + nbSites - 1) % nbSites),
                 payloadB, payloadC);
  ...
  EXPECT_EQ((myRank + nbSites - 1) % nbSites,
            sessionStub.getDelivered()[2].first);
  EXPECT_EQ(payloadB, sessionStub.getDelivered()[2].second->getPayload());

  EXPECT_EQ((myRank + nbSites - 1) % nbSites,
            sessionStub.getDelivered()[3].first);
  EXPECT_EQ(payloadC, sessionStub.getDelivered()[3].second->getPayload());

  // Check remainning waiting batches
  ASSERT_EQ(3,
            algoLayerRaw
                ->getWaitingBatchesNb());  // 3 = 1 non delivered + 2 from train

  EXPECT_EQ((myRank + 1) % nbSites,
            algoLayerRaw->getPreviousTrainsBatches()[1][0].senderPos);
  EXPECT_EQ(payloadD, algoLayerRaw->getPreviousTrainsBatches()[1][0]
                          .batchSessionMsg[0]
                          ->getPayload());

  // Check algo clocks
  ASSERT_EQ(4, algoLayerRaw->getClock(0));
  ASSERT_EQ(2, algoLayerRaw->getClock(1));
}
...
TEST_F(TrainsTest, ReceiveBatches) {
  ...
  EXPECT_EQ((myRank + 2) % nbSites,
            algoLayerRaw->getPreviousTrainsBatches()[0][0].senderPos);
  EXPECT_EQ(payloadD, algoLayerRaw->getPreviousTrainsBatches()[0][0]
                          .batchSessionMsg[0]
                          ->getPayload());

  EXPECT_EQ((myRank + 1) % nbSites,
            algoLayerRaw->getPreviousTrainsBatches()[0][1].senderPos);
  EXPECT_EQ(payloadA, algoLayerRaw->getPreviousTrainsBatches()[0][1]
                          .batchSessionMsg[0]
                          ->getPayload());
...
TEST_F(TrainsTest, FullTrainProcess) {
  ...
  ASSERT_EQ(4,
            algoLayerRaw
                ->getWaitingBatchesNb());  // 3 = 1 non delivered from train #1
                                           // + 2 from train #0 + 1 from outself
  ...
  ASSERT_EQ(
      2, algoLayerRaw->getPreviousTrainsBatches()[0][2].batchSessionMsg.size());
  EXPECT_EQ(payloadA, algoLayerRaw->getPreviousTrainsBatches()[0][2]
                          .batchSessionMsg[0]
                          ->getPayload());
  EXPECT_EQ(payloadC, algoLayerRaw->getPreviousTrainsBatches()[0][2]
                          .batchSessionMsg[1]
                          ->getPayload());
...
TEST_F(TrainsTest, LateTrain) {
  ...
  algoLayerRaw->callbackReceive(buildTrain(0));
  commLayerRaw->getSent()
      .clear();  // We clear the first train information which we already tested
                 // in previous execute() tests
  ASSERT_DEATH(algoLayerRaw->callbackReceive(buildTrain(0)),
               "Unexpected train #0 with clock 3. Algo clock: 4");
}
